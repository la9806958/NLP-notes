...
def main():
    ...
    if 'actual_return' not in final_results.columns:
        print("Warning: 'actual_return' column not found in FinalResults.csv")
    ...
    # For each return type in earnings_filtered, merge and analyze
    results = {}

    for return_col in earnings_return_cols:
        print(f"\n--- Analyzing {return_col} ---")

        # Merge dataframes
        merged_df = pd.merge(
            final_results[['ticker', final_return_col, 'research_note']],
            earnings_filtered[['ticker', return_col]],
            left_on=['ticker', final_return_col],
            right_on=['ticker', return_col],
            how='inner'
        )
        ...
        if merged_df.empty:
            print(f"No matches found for {return_col}")
            continue
        ...
        # Convert scores to directions
        merged_df['score_direction'] = merged_df['directional_score'].apply(direction_from_score)
        merged_df['return_direction'] = merged_df[return_col].apply(direction_from_return)

        # Remove rows with missing values
        valid_data = merged_df.dropna(subset=['score_direction', 'return_direction'])
        ...
        # Print some examples
        print("\nFirst 5 examples:")
        print(valid_data[['ticker', 'directional_score', 'score_direction', return_col, 'return_direction']].head())
