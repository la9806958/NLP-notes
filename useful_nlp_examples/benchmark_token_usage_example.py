...
CSV_OUTPUT_FILE = 'financial_analysis_4o_mini_results.csv'
CSV_HEADERS = [
...
def load_and_join_data():
    ...
    broker_df = pd.read_csv('broker_info_extraction_4o_mini_final_enhanced.csv')
    ...
    ticker_df = pd.read_csv('extracted_tickers.csv')
...
def save_results(results):
    ...
    if results:
        ...
        output_file = 'financial_analysis_4o_mini_results.csv'
        df.to_csv(output_file, index=False)
        logger.info(f"Results saved to {output_file}")

        # Also save a human-readable version
        with open('financial_analysis_4o_mini_readable.txt', 'w') as f:
...
def main():
    ...
    try:
        ...
        print(f"📖 Readable results in: financial_analysis_4o_mini_readable.txt")
